...
describe('ContentScriptManager', () => {
  ...
  beforeEach(() => {
    ...
    (TimeDisplayPill as jest.Mock).mockImplementation(() => mockTimeDisplayPill);
  ...
  describe('Initialization', () => {
    it('should initialize successfully', async () => {
      ...
      expect(mockMessageRouter.requestSessionState).toHaveBeenCalledWith('example.com');
    });
    ...
    it('should handle initialization errors', async () => {
      ...
      await expect(contentManager.initialize()).rejects.toThrow('Initialization failed');
  ...
  describe('Domain Management', () => {
    ...
    it('should handle URL changes', () => {
      ...
      expect(mockMessageRouter.requestSessionState).toHaveBeenCalledWith('newdomain.com');
    });

    it('should ignore URL changes within same domain', () => {
      const initialCallCount = mockMessageRouter.requestSessionState.mock.calls.length;

      contentManager.handleUrlChange('https://example.com/different-page');

      expect(contentManager.getDomain()).toBe('example.com');
      expect(mockMessageRouter.requestSessionState).toHaveBeenCalledTimes(initialCallCount);
    });
  ...
  describe('Message Handling', () => {
    ...
    beforeEach(async () => {
      ...
      sessionUpdateHandler = registerCalls.find(call => call[0] === 'SESSION_UPDATE')?.[1];
      settingsChangeHandler = registerCalls.find(call => call[0] === 'SETTINGS_CHANGE')?.[1];
    });

    it('should register message handlers', () => {
      expect(mockMessageRouter.registerHandler).toHaveBeenCalledWith('SESSION_UPDATE', expect.any(Function));
      expect(mockMessageRouter.registerHandler).toHaveBeenCalledWith('SETTINGS_CHANGE', expect.any(Function));
    });
    ...
    it('should handle session update messages', async () => {
      ...
      expect(mockTimeDisplayPill.onSessionUpdate).toHaveBeenCalledWith(sessionMessage.payload);
    });
    ...
    it('should handle settings change messages', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockTimeDisplayPill.onSettingsChange).toHaveBeenCalledWith(settingsMessage.payload);
    });
  ...
  describe('Initial State Request', () => {
    it('should request initial state successfully', async () => {
      ...
      expect(mockMessageRouter.requestSessionState).toHaveBeenCalledWith('example.com');
      expect(mockTimeDisplayPill.onSessionUpdate).toHaveBeenCalledWith(mockResponse.data);
    });
    ...
    it('should handle request errors', async () => {
      mockMessageRouter.requestSessionState.mockRejectedValue(new Error('Network error'));
  ...
  describe('Error Handling', () => {
    ...
    it('should report component broadcast errors', () => {
      ...
      expect(async () => await sessionUpdateHandler(sessionMessage)).not.toThrow();
    });
  });
